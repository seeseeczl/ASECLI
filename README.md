...
项目当前是内部专有工具，正式版本通过私有 GitHub Release 分发；未上传 PyPI、CLI Hub 或公开包仓。试用者必须先获得 `seeseeczl/ASECLI` 私有仓库权限。当前版本为 `v0.3.0`；需要回滚时可安装不可变的 `v0.2.0` 或 `v0.1.0`。
...
```bash
uv tool install --force ./asecli-0.3.0-py3-none-any.whl
```
...
正式版本与校验资产见 [ASECLI v0.3.0（内部正式版）](https://github.com/seeseeczl/ASECLI/releases/tag/v0.3.0)。该链接和下载命令仅对已获私有仓库权限的账号可用。`v0.2.0` 与 `v0.1.0` 仍保留为不可变回滚点。
...
CI 通过只证明远端自动门禁通过。进入“已交付”还需要真实的 push run 链接、可下载 artifact 与 hash 核对、以及需要时的回滚观察。项目禁止自动发布；每次 GitHub Release、PyPI、Hub 上传或对外分发均须获得单独书面授权。`v0.3.0` 按用户 2026-09-03 的明确授权发布为私有正式 Release，后续版本不会因此自动发布。
