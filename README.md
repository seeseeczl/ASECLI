...
当前正式版：**[0.8.2](https://github.com/seeseeczl/ASECLI/releases/tag/v0.8.2)** · [PyPI](https://pypi.org/project/asecli/0.8.2/) · [Agent 操作手册](skills/asecli/SKILL.md) · [SG 转换后整理流程](skills/asecli/references/conversion-workflow.md)
...
当前正式版本为 `0.8.2`（精确源 Alpha 混合、资源快照与转换认证加固），可从 [PyPI](https://pypi.org/project/asecli/0.8.2/) 或 [GitHub Release](https://github.com/seeseeczl/ASECLI/releases/tag/v0.8.2) 安装。历史版本见 [GitHub Releases](https://github.com/seeseeczl/ASECLI/releases)，`v0.8.0` 保留为回滚点。
...
uv tool install /path/to/asecli-0.8.2-py3-none-any.whl
asecli --help
```
...
ASECLI 随 wheel 发布自己拥有的 EditorGraphSpec v3 Schema，并固定一份 SGCLI `0.3.5` 的 `sgcli.native.v3` 生产快照；SGCLI 同样固定 ASECLI `0.8.2` 快照。双仓兼容检查会比较两组原始 Schema SHA-256，任何漂移都必须先升级生产者再放行。
...
```json
{"contract_version":1,"cli_version":"0.8.2","command":"parse","ok":true,"data":{"node_count":7}}
{"contract_version":1,"cli_version":"0.8.2","command":"parse","ok":false,"error":{"code":"NOT_FOUND","message":"..."}}
{"contract_version":1,"cli_version":"0.8.2","command":"version","ok":true,"data":{"version":"0.8.2"}}
```
