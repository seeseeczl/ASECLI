...
- ASECLI 任务只修改本仓库；不得因转换、测试失败或发布而自动修改、提交、推送 SGCLI。对方缺能力时记录协议需求，由用户另行授权对方任务。
- 运行、构建、必需测试、发布和安装不得依赖对方源码、开发目录、可执行文件、候选分支或跨仓凭据。
- 允许携带版本化公开 JSON Schema 静态快照；摘要用于本包完整性，不要求等于对方当前 main。未知语义继续失败关闭。
- 跨产品验收属于另行授权的外部黑盒编排，不成为本仓发布前置，不回写对方。
