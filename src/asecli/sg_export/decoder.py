...
from ..core import (AseFile, COMMENTARY_TYPE, GET_LOCAL_VAR_TYPE,
                    REGISTER_LOCAL_VAR_TYPE, parse_get_local_var)
...
def build_candidate(
    ...
    *,
    ...
    report = report_template(source, target, ase.graph.version, raw)
    ...
    report["target_mapping"] = target_spec
    ...
    for wire in ase.graph.wires:
        ...
        record_port(report, source_id, source_port, source_end, wire.in_node, int(wire.in_port), target_end)
    for node in nodes:
    ...
    report["output_bindings"] = [
        ...
        for edge in edges
        if edge.target_id.startswith(("SurfaceDescription.", "VertexDescription."))
    ]
    ...
    candidate = {"schema": "sgcli.native.v3", "name": name or f"Converted/{source.stem}",
