...
from ..core import NodeLine, WireLine
from .custom_expression import convert_custom_expression
from .model import SemanticNode, finite, numbers, position
from .sources import property_spec, resolve_asset, sampler_texture_guid
...
DIRECT_RULES = {
    ...
    "AmplifyShaderEditor.PowerNode": ("power", {0: "A", 1: "B"}, {0: "Out"}),
...
CUSTOM_EXPRESSION_TYPE = "AmplifyShaderEditor.CustomExpressionNode"
def convert_node(node: NodeLine, wires: list[WireLine], compiled: dict, target: Path,
    ...
    if node.type_name == CUSTOM_EXPRESSION_TYPE:
        return convert_custom_expression(node, settings)
    if node.type_name == "AmplifyShaderEditor.WorldPosInputsNode":
        item = SemanticNode(
            node.node_id,
            f"ase_{node.node_id}",
            "position",
            position(node),
            output_names={0: "Out"},
            settings={**settings, "spacePopup": "World"},
        )
        return [item], [], {0: (item.target_id, 0)}
    if node.type_name == "AmplifyShaderEditor.WorldNormalVector":
        item = SemanticNode(
            node.node_id,
            f"ase_{node.node_id}",
            "normal",
...
def _constant_node(node, wires, compiled, settings):
    ...
    if parameter == "Property":
        ...
        base = SemanticNode(node.node_id, base_id, "property", position(node),
                            output_names={0: prop["display_name"]}, property_name=prop["name"])
    else:
        channels = "XYZW"[:width]
...
def _texture_node(node, wires, compiled, target, asset_map, settings):
    ...
    try:
        create = fields.index("Create")
        dimension = fields.index("Texture2D", create)
    except ValueError as exc:
        raise ValueError("texture dimension is not certified") from exc
    if fields[dimension - 4] not in {"", "False"}:
        raise ValueError("texture dimension or unpack mode is not certified")
    if fields[dimension - 1] not in {"Auto", "0"}:
        raise ValueError(f"texture mip mode {fields[dimension - 1]!r} is not certified")
    guid = sampler_texture_guid(node)
...
def _constant_value(node, width):
    fields = node.raw_fields
    if len(fields) > 6 and fields[6] == "Property" and "Create" in fields:
        index = fields.index("Create") + 8
        if index >= len(fields):
            raise ValueError("property default is missing")
        return finite(fields[index]) if width == 1 else numbers(fields[index], width)
...
def _input_defaults(node):
    ...
    fixed = {"AmplifyShaderEditor.LerpOp": 3, "AmplifyShaderEditor.ClampOpNode": 3,
             ...
             "AmplifyShaderEditor.PowerNode": 2}
    count = fixed.get(node.type_name)
    start = 2 if node.type_name in {
        "AmplifyShaderEditor.BreakToComponentsNode",
