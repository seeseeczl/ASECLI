...
| FR-0007 | 功能 | CLI JSON 输出契约：除显式 help 外 stdout 恒为严格 JSON（含 contract/CLI version、command、ok），统一错误码 | 所有子命令契约测试通过 | REG-0007 | 已确认 |
...
导出必须保留算法或采用已认证数学映射，并对账变量名、类型、默认值、范围、资源和 Shader 设置。已识别 AlphaModulate 的局部浮点精度边界单独提示；浮点逐位或视觉未验证不阻断 JSON。错误连接、混合和无法表示的运行时属性继续失败关闭。关联 TASK-0067、REG-0064。
