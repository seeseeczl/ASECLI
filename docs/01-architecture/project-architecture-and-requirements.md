...
| FR-0008 | 功能 | 节点精排：保留旧分层模式；可选使用 ASE 真实节点/标题/端口几何，以 Output 为根从右向左递归展开上游子树，按输入端口顺序居中，并自内向外收紧 Comment | 布局不改连线、参数或 Local Var；父级端口组与子树带中心误差 ≤8px；节点/子树/Comment 零重叠和零越界；硬门禁失败不写盘；同输入确定性且第二次移动为零 | REG-0012 REG-0050 | 已确认 |
...
| MOD-BRIDGE | src/asecli/bridge | MCP 传输、URL/token 信任边界、tool result、重编译、受控 Editor 图创建、确定性资源拼装与 GUI 支持升级 | `McpClient` / `recompile_via_mcp` / `create_shader_via_mcp` / `inspect_gui_support` / `install_gui_support` | 标准库；禁止依赖 cli | FR-0004 FR-0009 FR-0011 CR-0008 CR-0012 CR-0014 ADR-0002 ADR-0007 ADR-0012 ADR-0013 ADR-0015 ADR-0016 | long |
...
| MOD-LAYOUT | src/asecli/core/layout.py; meticulous_layout.py; layout_audit.py; comment_layout.py; wire_geometry.py | 兼容分层布局；脑图式 DAG 主树排版、次级边/Local Var/Comment 质量审计与布局结果所有权 | `layout_positions`；`meticulous_layout_positions`；`audit_meticulous_layout` | 允许依赖 core 模型；禁止依赖 schema/bridge/cli | FR-0008 ADR-0005 CR-0019 | long |
