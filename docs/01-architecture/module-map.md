...
| MOD-CUSTOM-GUI | src/asecli/core/custom_gui.py; material_gui_spec.py | FR-0009 FR-0010 | 主 Master/ShaderLab CustomEditor 同步；PropertyNode MZGUI 尾部、UTF-16 文案编码、属性名定位和批量规范 | `inspect_custom_gui`；`resolve_property_node`；`apply_material_gui_spec` | 仅依赖 MOD-CORE model；禁止依赖 schema/check/bridge/cli |
| MOD-GUI-SUPPORT | src/asecli/bridge/gui_support.py; bridge/resources/asecli_material_gui.cs.txt | FR-0009 | 原生 MZGUI 检测、内置 GUI 固定路径安装、Foldout/Tooltip/HelpBox 解释与动态技术 Tooltip | `inspect_gui_support`；`install_gui_support`；`ASECLI.MaterialGUI.ASECLIMaterialGUI` | Python 仅标准库；C# 仅 UnityEditor/UnityEngine；禁止依赖 ASE API |
