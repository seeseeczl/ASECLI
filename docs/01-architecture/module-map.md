...
| MOD-LAYOUT | src/asecli/core/layout.py; meticulous_layout.py; layout_audit.py; comment_layout.py; wire_geometry.py | FR-0008 CR-0019 | 兼容分层布局；基于真实几何的递归 DAG 子树居中、Local Var 就近、Comment 收框、线路与空白率审计 | `layout_positions`；`meticulous_layout_positions`；`audit_meticulous_layout` | 允许依赖 core 模型；禁止依赖 schema/bridge/cli |
