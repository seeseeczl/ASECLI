...
| TASK-0037 | CR-0006 AUD-CI-001 | 将三个 GitHub Action 升级到官方 Node 24 版本并以固定 SHA allowlist 防回退 | 官方 tag/API 只读核对；TASK-0034 | `.github/workflows/ci.yml`；tools/check_ci_governance.py；tests/test_governance.py；supply-chain-policy | 升级 Python 依赖；改变 job；推送或重发 Release | TASK-0034/long/2026-09-03 | REG-0043；CI governance/供应链/pytest；真实 run 全日志无 Node 20/Node 弃用警告 | runner 不满足 `>=2.327.1`、artifact 行为改变或仍输出弃用警告时继续阻塞 | run 33713836753 全绿并清除 Node 20 强制警告，但 setup-uv v7.1.6 仍有 `DEP0040`/`DEP0169`；已升级 v10.0.1，第二次远程 run 待验证 | 远程复验中 |
