...
owner: long
related: [ARCH-REQ-0001, PRJ-ASECLI, FR-0009, FR-0010, FR-0011, CR-0007, CR-0008, CR-0010, CR-0011, CR-0012, BUG-0015, BUG-0017, BUG-0018, AUD-20260901]
supersedes: 无（首次启动）
...
| TASK-0032 | FR-0005 FR-0010 FR-0011 CR-0012 BUG-0016 BUG-0017 BUG-0018 | 将中文显示名、自动变量名/默认值 Tooltip 和中文说明固化为所有 CLI 创建文件的强制契约，并闭合当前 MCP v2 创建回执及 0.2.0 CI 打包/校验路径 | 用户 2026-09-02 明确要求；TASK-0031 | core/property_presentation.py；custom_gui/material spec；EditorGraphSpec；create/写入门禁；CI/SBOM/SHA256SUMS；tests；README/治理 | 自动编造中文语义；修改生产 Shader；把 v1 真实 Editor 证据冒充 v2；依赖 Agent 规则；透传任意 C#；在 CI 硬编码历史包版本或工作区路径 | TASK-0031/long/2026-09-02 | REG-0037 REG-0038 REG-0039 REG-0040；双 Python全量、REG/CI governance/供应链/Skill/构建；当前团结 v2 创建/recompile/Inspector；远端 package 与下载 artifact 校验 | 契约可被绕过、MCP 失败被误报成功、非合规 create 能落盘、历史非 ASECLI 文件无法维护、CI 安装错误版本或下载清单不可直接校验时停止 | 新用例先失败 5 项；双 Python 223 passed/3 skipped；REG 36 条、CI governance、供应链、Skill、可复现 0.2.0 构建与隔离安装/回滚通过；Caster 1 属性/Receiver 6 属性零违规；当前团结 v2 创建与 recompile 后两属性零差异；Inspector 中文显示名/说明可见，用户现场确认 Tooltip 正常；验证资产已清理；strict 与 HEAD 同为 31 项历史问题；首次推送依次暴露旧版本硬编码和 SHA256SUMS 路径前缀，均已最小修复并等待最终远端 artifact 复验；新进程重开按用户约束未执行；0.2.0 已推送但尚未打 tag 或发布 | 已验证（限当前实例，待最终远端 artifact 复验与发布） |
