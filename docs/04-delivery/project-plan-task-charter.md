...
| TASK-0011 | FR-0004 CR-0001 | MCP 桥接：recompile_via_mcp（execute_code 注入 ASE 重编译序列） | TASK-0001 实验结论 | src/asecli/bridge/**；tests/test_bridge_recompile.py | 开放任意代码执行入口；自建 IPC | TASK-0001/long/2026-09-08 | REG-0005：触发后目标 shader HLSL 变更且 CHKSM 更新；命令：`uv run pytest -m bridge tests/test_bridge_recompile.py` | Codely 调用链失败时停止并记录 | 提交 hash 回填 | 代码完成 |
