...
- [ ] P1.2 补跑 Editor v2 新进程重开并归档脱敏证据 — **阻塞**（2026-09-02）：当前环境无 ASE 1.9.6.2 隔离工程，无法执行；明天到公司再跑
...
- 正在做：无
- 下一步：公司环境具备 ASE 1.9.6.2 隔离工程后重跑 P1.2
- 阻塞/风险：P1.2 已标记阻塞。2026-09-02 本机无 ASE 1.9.6.2；已装团结为 2022.3.62t7/62t13，已有工程 ASE 均为 1.9.1.9。执行器硬门禁只接受 1.9.6.2，不得用 1.9.1.9 冒充，不得改写 Genesis/Baking/HDRP 生产工程。恢复条件：隔离工程路径 + 对应团结可执行文件。
...
- 计划外问题处理规则：记录，不展开；只有阻塞 P1 时暂停并请求确认
- 当前状态：**阻塞**（2026-09-02 记录；用户确认明天到公司再执行）
- 阻塞原因：当前环境没有可用的 ASE 1.9.6.2 隔离工程。本机已装团结为 `2022.3.62t7` / `2022.3.62t13`（无文档中的 `2022.3.61t9`）；已有带 ASE 的工程均为 1.9.1.9。C# 执行器硬门禁要求 `aseVersion == "1.9.6.2"`。
- 禁止项：不得把 Genesis/Baking/HDRP 当隔离工程写入；不得用 1.9.1.9 冒充 1.9.6.2。
- 恢复条件：公司环境提供含 ASE 1.9.6.2 的隔离工程路径，以及对应团结可执行文件。
- 恢复后动作：只在隔离副本创建临时 shader → 独立 recompile / SHA-256 → 完全退出后新进程 `LoadFromDisk` → 核对 manifest 与 `property_presentation.valid` → 删除验证资产 → 回写 REG-0037。
