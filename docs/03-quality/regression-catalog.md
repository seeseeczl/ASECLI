...
| REG-0021 | CR-0006 AUD-OPS-001 AUD-SUPPLY-001 | supply-delivery：CI governance、lock/Action/secret/license、SPDX 与可复现构建 | `uv run pytest tests/test_supply_chain.py` + 本地双构建/安装回滚 | 本地已验证；远程 CI 未运行（无 push 授权） |
| REG-0022 | FR-0009 | custom-gui-native-roundtrip：真实 HLIT/MZGUI_Test 格式读取；主 Master/编译指令同步；中文显示名读取；变量名/Shader 默认值 Tooltip、分组、HelpBox 原生编码与数量写回；dry-run/备份/CHKSM/注入/非 Property 失败路径 | `uv run pytest tests/test_custom_gui.py` | 自动通过；ASE 1.9.6.2 MZGUI_Test 隔离副本读写通过，真实材质 Inspector UI 待目标工程验收 |
| REG-0023 | FR-0009 FR-0010 | material-gui-spec-atomic：属性名定位；JSON 顺序/中文分组/技术 Tooltip/常驻说明原子写回；未列属性稳定追加；重复/未知/错误类型/非 MZGUI/dry-run/备份/CHKSM | `uv run pytest tests/test_custom_gui.py` | 自动通过；中文 `inspector_name` 由 EditorGraphSpec 契约覆盖；真实材质 Inspector UI 待目标工程验收 |
...
| REG-0030 | FR-0009 | gui-support-provider-and-runtime：原生 MZGUI 优先；缺失时 dry-run/固定路径安装/幂等；冲突拒绝覆盖；双 CustomEditor 接受同一 Foldout/Tooltip/HelpBox 协议；内置层自动读取变量名和 Shader 默认值 | `uv run pytest tests/test_gui_support.py tests/test_custom_gui.py tests/test_cli_contract.py` + 隔离 Unity 2021.3 BatchMode 编译/attribute/default-value verifier | 自动 56 passed；真实 Unity C# 编译通过并读到三类 attribute 与 `_Value=1.25`；目标 Inspector 交互/视觉待验 |
| REG-0031 | CR-0006 BUG-0010 AA-REL-001 | build-reproducibility-diagnostics：构建输出不进入 sdist；wheel/sdist 双构建一致；失败生成 hash/gzip/tar 成员差异 | `uv run pytest tests/test_build_reproducibility.py` + 临时 clone 仿真 CI 双构建 | 自动与本地双构建通过；远程 runner 待验 |
| REG-0032 | FR-0009 BUG-0011 AA-DATA-001 | custom-gui-version-matrix：19109 仅使用真实 Master 契约；19602 支持 MZGUI；未知未来版本及尾随数字歧义写前失败且输入不变 | `uv run pytest tests/test_custom_gui.py` | 自动通过 |
...
| REG-0035 | FR-0009 BUG-0014 AA-COMPAT-001 | gui-provider-detection-hardening：全限定/global/alias 基类、任意 DLL 候选与 Editor 反射；unknown 阻止安装 | `uv run pytest tests/test_gui_support.py` | 自动通过；真实反射待目标 Editor |
