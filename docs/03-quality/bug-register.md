...
| BUG-0022 | 用户反馈 / 真实 MZGUI 与 ASE 源码对照 / 对抗审计 AA-DATA-001 AA-COR-001 | fallback 保存丢属性、缺可视化 authoring、旧私有 Editor 名不可移植；进一步发现原生 MZGUI 状态双存储和 Apply 半提交 | REG-0047 | EditorWindow + hydration；统一 `MZGUI.MZGUI`；旧名 canonical 化；原生状态 reconciliation；Apply 内存/磁盘事务；V2 provider 与可恢复 handoff | 自动已解决：`258 passed, 3 skipped`、真实 native probe、两种拼装源内存编译；真实无 MZGUI UI 与跨环境保存重开待验 |
| BUG-0023 | SGCLI P0.2 真实 MCP codedom | delegate result 遮蔽、System.Tuple 程序集冲突及方法体内 InputTarget 类型声明导致 create 编译失败 | REG-0059 | values 局部变量和 KeyValuePair 节点/端口对；固定片段不声明类型 | 已修复；真实 property_texture/defaults_degradation create、manifest、validate、recompile 通过；SphereMask 的 Radius 重载错误为已知独立限制 |
