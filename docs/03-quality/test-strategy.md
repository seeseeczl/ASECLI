...
- REG-0026～0028 的 pytest/mock 不证明 ASE 私有字段或 GUI 生命周期在目标版本可用；REG-0029 已在隔离团结 2022.3.61t9 + ASE 1.9.6.2 完成 v1 创建与新进程回读，证明当前白名单结构可编辑；REG-0037/0038 已在当前同版本团结实例完成 v2 属性呈现后处理、重编译与 Inspector 验收。目标工程运行时矩阵、材质绑定、平台编译、最终画面及 v2 新进程重开仍需独立验收。
