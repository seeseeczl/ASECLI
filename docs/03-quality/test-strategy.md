...
13. **Editor API 创建**：底层 v1 继续覆盖白名单、端口、参数编码和已验证的隔离团结结构 E2E；正式 CLI 接受 EditorGraphSpec v2/v3，并在 MCP 前强制每个 Property/Sampler 的中文 `inspector_name/tooltip`，`help` 可选。v3 额外覆盖 primitive 闭包、recipe 原生展开/Custom Expression 降级、精度、默认值、范围、未知版本与前向引用失败关闭。ASE 暂存 Save/Load 与真实 manifest 对账后提交，CLI 写入 MZGUI 元数据并执行文件级契约复验；目标图由新 Editor 进程重载。真实 Inspector、平台编译和渲染仍单独记录，不能复用结构 E2E 结论。
